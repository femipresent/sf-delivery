...
const DriverLayout = ({ children, onLogout }) => {
  ...
  const [stats, setStats] = useState({ todayEarnings: 0, weeklyEarnings: 0, completedJobs: 0, rating: 0 });

  useEffect(() => {
    const fetchDriverData = async () => {
      try {
        const [profileRes, jobsRes] = await Promise.all([
          API.get('/register/profile'),
          API.get('/driver/bookings')
        ]);
        const profile = profileRes.data.data;
        setDriverData(profile);
        const bookings = jobsRes.data.data || [];
        setJobs(bookings);
        const active = bookings.find(b => b.status === 'assigned' || b.status === 'picked_up' || b.status === 'in_transit');
        setCurrentJob(active || null);
        const delivered = bookings.filter(b => b.status === 'delivered');
        setStats({
          todayEarnings: delivered.filter(b => new Date(b.updatedAt).toDateString() === new Date().toDateString()).reduce((s, b) => s + (b.pricing?.total || 0), 0),
          weeklyEarnings: delivered.reduce((s, b) => s + (b.pricing?.total || 0), 0),
          completedJobs: delivered.length,
  ...
  const driver = {
    name: driverData ? `${driverData.firstName} ${driverData.lastName}` : 'Driver',
    id: driverData?._id?.slice(-6).toUpperCase() || 'N/A',
    phone: driverData?.phone || '',
    email: driverData?.email || '',
    vehicle: {
      type: driverData?.vehicleInfo || 'N/A',
      plate: driverData?.vehicleInfo?.split('-')[1]?.trim() || 'N/A',
      capacity: 'N/A', color: 'N/A', year: 'N/A',
      fuelType: 'N/A', mileage: 'N/A', lastService: 'N/A', nextService: 'N/A', insuranceExpiry: 'N/A'
    },
    rating: stats.rating,
    completedJobs: stats.completedJobs,
    todayEarnings: stats.todayEarnings,
    weeklyEarnings: stats.weeklyEarnings,
    monthlyEarnings: stats.weeklyEarnings
  };

  const notifications = [
    ...(currentJob ? [{ id: 1, title: 'Active Job', message: `You have an active delivery in progress`, time: 'Now', type: 'job', icon: <TruckIcon className="w-4 h-4" />, unread: true }] : []),
    ...(jobs.filter(b => b.status === 'assigned').length > 0 ? [{ id: 2, title: 'New Job Assigned', message: `${jobs.filter(b => b.status === 'assigned').length} job(s) waiting for pickup`, time: 'Recent', type: 'job', icon: <TruckIcon className="w-4 h-4" />, unread: true }] : []),
  ];
  ...
  useEffect(() => {
    if (!isLoading) {
      // data already loaded in the main useEffect
    }
  }, [isLoading]);
        ...
        <div className="mb-4 md:mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3 lg:gap-4 mb-4 md:mb-6">
            <StatCard
              ...
              suffix="₦"
              description={`${jobs.filter(b => b.status === 'delivered' && new Date(b.updatedAt).toDateString() === new Date().toDateString()).length} deliveries today`}
            />
            <StatCard
              title="Weekly Total"
              ...
              color={{ bg: 'bg-blue-50', text: 'text-blue-600' }}
              suffix="₦"
              description={`${stats.completedJobs} total deliveries`}
            />
            <StatCard
              title="Driver Rating"
              value={driver.rating || 'N/A'}
              icon={ShieldCheckIcon}
              color={{ bg: 'bg-blue-50', text: 'text-blue-600' }}
              description={`${stats.completedJobs} deliveries`}
            />
            <StatCard
              title="Active Job"
              ...
              icon={TruckIcon}
              color={{ bg: 'bg-blue-50', text: 'text-blue-600' }}
              description={currentJob ? `Booking: ${currentJob.trackingNumber}` : 'Available for work'}
            />
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-4 md:space-y-5 lg:space-y-6">
              ...
              {currentJob ? (
                <div className="space-y-4">
                  <div className="bg-blue-50 rounded-xl p-4">
                    ...
                    <p className="font-bold text-gray-900">{currentJob.trackingNumber}</p>
                  </div>
                  <div className="space-y-3">
                    <div className="flex items-start">
                      ...
                      <div className="ml-3">
                        ...
                        <p className="text-sm font-medium">{currentJob.pickup?.address?.street}, {currentJob.pickup?.address?.city}</p>
                      </div>
                    </div>
                    <div className="flex items-start">
                      ...
                      <div className="ml-3">
                        ...
                        <p className="text-sm font-medium">{currentJob.delivery?.address?.street}, {currentJob.delivery?.address?.city}</p>
                      </div>
                    </div>
                  </div>
                  <button onClick={() => setActiveTab('navigation')} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 rounded-full flex items-center justify-center">
                    <MapIcon className="w-4 h-4 mr-2" /> Navigate to Delivery
                  </button>
                </div>
              ) : (
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-4 md:p-6 shadow-sm">
              ...
              <div className="space-y-2 md:space-y-3">
                {jobs.filter(b => b.status === 'delivered').slice(0, 3).map((job) => (
                  <div key={job._id} className="flex items-center justify-between p-3 md:p-4 border border-gray-100 rounded-lg md:rounded-xl hover:bg-gray-50">
                    <div className="min-w-0">
                      <p className="font-medium text-gray-900 text-sm truncate">{job.trackingNumber}</p>
                      <p className="text-xs text-gray-600 truncate">{job.delivery?.address?.city}</p>
                    </div>
                    <div className="text-right ml-2 flex-shrink-0">
                      <p className="font-medium text-gray-900 text-sm">₦{(job.pricing?.total || 0).toLocaleString()}</p>
                    </div>
                  </div>
                ))}
                {jobs.filter(b => b.status === 'delivered').length === 0 && (
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-3 md:p-4 lg:p-5 border-t">
            <div className="flex items-center mb-3 md:mb-4">
              <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-blue-600 flex items-center justify-center border-2 border-blue-500">
                <span className="text-white font-bold text-xs">{driver.name.split(' ').map(n=>n[0]).join('').slice(0,2)}</span>
              </div>
              <div className="ml-2 md:ml-3">
        ...
        <div className="p-5 border-t">
          <div className="flex items-center mb-4">
            <div className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center border-2 border-blue-500">
              <span className="text-white font-bold text-sm">{driver.name.split(' ').map(n=>n[0]).join('').slice(0,2)}</span>
            </div>
            <div className="ml-3">
              <p className="text-sm font-semibold text-gray-900">{driver.name}</p>
              <p className="text-xs text-gray-500">Driver</p>
          ...
          <div className="flex items-center justify-between h-14 md:h-16 px-3 md:px-4 lg:px-6">
            ...
            <div className="flex items-center space-x-2 md:space-x-3 lg:space-x-4">
              ...
              >
                ...
                <div className="w-8 h-8 md:w-9 md:h-9 lg:w-10 lg:h-10 rounded-full bg-blue-600 flex items-center justify-center border-2 border-blue-500">
                  <span className="text-white font-bold text-xs">{driver.name.split(' ').map(n=>n[0]).join('').slice(0,2)}</span>
                </div>
              </button>
            </div>
